...
(function(factory) {
	...
	var defaults = {
		...
		footer: true,
		dayHeader: true,
		mode: 'page',			// 'widget' (default), 'tiny', 'popup', 'page'
		animDuration: 200,
		transition: '',
	...
	$.fn.supercal = function(method) {
		...
		var pMethods = {
			...
			drawMonth: function(date, options) {
				...
				current.setDate(now1.getDate() - 0);
				var prevmonthdate = new Date();
				prevmonthdate.setDate(now1.getDate() - 30);
				var nextmonthdate = new Date();
				nextmonthdate.setDate(now1.getDate() + 30);

				// Add previous month's days
				for(var i = 1; i <= numPrevDays; i++) {
					var day = (daysInLastMonth - numPrevDays) + i;
					var curdate = new Date(date.getFullYear(), date.getMonth() - 1, day, 0, 0, 0);
					var greyclass;
					if (curdate >= current && curdate < nextmonthdate) {
						greyclass ='';
					}
					else {
						greyclass ='month-prev';
					}
				...
				for(var i = 1; i <= numCurrentDays; i++) {
					...
					if (day.date > current && day.date < nextmonthdate) {
						day.classes ='';
					}
...
//						day.classes ='selected';
				...
				for(var i = 1; i <= numNextDays; i++) {
					var day = i;
					var curdate = new Date(date.getFullYear(), date.getMonth() + 1, i, 0, 0, 0);
					var greyclass;
					if (curdate >= current && curdate < nextmonthdate) {
						greyclass ='';
					}
